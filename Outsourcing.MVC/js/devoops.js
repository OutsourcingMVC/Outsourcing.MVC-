...
//
//  Dynamically load  jQuery Datepicker plugin
//  homepage: http://trentrichardson.com/examples/timepicker/
//
//function LoadJqueyDatePickerScript(callback) {
//    if (!$.fn.datePicker) {
//        $.getScript('plugins/jquery-datepicker/jquery.DatePicker.min.js', callback);
//    }
//    else {
//        if (callback && typeof (callback) === "function") {
//            callback();
...
//使用POST方式请求服务器
function LoadAjaxContentWithPost(url) {
    //debugger;
    $('.preloader').show();
    $.ajax({
        mimeType: 'text/html; charset=utf-8', // ! Need set mimeType only when run from local file
...
function TestTableWage() {
    $('#datatable-Wage').dataTable({
        "aaSorting": [[0, "asc"]],
        "sDom": "T<'box-content'<'col-sm-6'f><'col-sm-6 text-right'l><'clearfix'>>rt<'box-content'<'col-sm-6'i><'col-sm-6 text-right'p><'clearfix'>>",
        "sPaginationType": "bootstrap",
        "oLanguage": {
